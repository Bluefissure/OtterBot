...
def group_ban(bot,group_id,user_id,duration=60):
    json_data = {"group_id":group_id,"user_id":user_id,"duration":duration}
    bot.send_ws_api("set_group_ban",json_data)
    time.sleep(1)
...
def delete_msg(bot,message_id):
    json_data = {"message_id":message_id}
    bot.send_ws_api("delete_msg",json_data)
    time.sleep(1)
...
def MyJsonResponse(res):
    qqbots = QQBot.objects.filter(user_id=BOT_QQ)
    if(len(qqbots)==0):
        print("no such bot in db")
        return
    if(GROUP_BAN_TILL and time.time()<GROUP_BAN_TILL):
        return
    qqbot = qqbots[0]
    if time.time() < qqbot.last_reply_time+FREQUENCY_LIMIT:
        print("pass")
        return
    else:
        qqbot.last_reply_time = time.time()
        qqbot.save()
    return JsonResponse(res)


def calculate_add_delay(user_id):
    now_time = int(time.time())
    try_time = 0
    while(try_time<100):
        f = int(md5(str(user_id).encode()).hexdigest(),16) % 10 == int(md5(str(now_time//(24*3600)).encode()).hexdigest(),16) % 10 
        if f:
            return try_time
        now_time += 24*3600
        try_time += 1
...
def allow_add(user_id):
    return calculate_add_delay(user_id) == 0


@require_websocket
def qqpost_ws_api(req):
    ws_self_id = req.META.get('HTTP_X_SELF_ID')
    ws_client_role = req.META.get('HTTP_X_CLIENT_ROLE')
    ws_access_token = req.META.get('HTTP_AUTHORIZATION').replace("Token","").strip()
    print("%s:%s:%s"%(ws_self_id, ws_access_token, ws_client_role))
    bots = QQBot.objects.filter(user_id=ws_self_id,access_token=ws_access_token)
    if(len(bots) == 0):
        print("%s:%s:AUTH_FAIL"%(ws_self_id, ws_access_token))
        return
    if(len(bots) > 1):
        print("%s:%s:MULTIPLE_AUTH"%(ws_self_id, ws_access_token))
        return
    bot = bots[0]
    ws = req.websocket
    if(ws_client_role=="API"):
        while(True):
            time.sleep(1)
            # print("%s:API heartbeat"%(ws_self_id))
            bot.refresh_from_db()
            bot.api_last_time = int(time.time())
            api_data = json.loads(bot.api_data)
            if(type(api_data)==dict):
                api_data = [api_data]
            for item in api_data:
                time.sleep(1)
                print("%s API read:%s"%(ws_self_id, item))
                ws.send(str(json.dumps(item)).encode())
                print("%s API sent:%s"%(ws_self_id, item))
            bot.clear_api_data()
                    ...
                    if("echo" in receive.keys()):
                        ...
                        if(echo.find("get_group_member_list")==0):
                            ...
                            if(len(groups)>0):
                                ...
                                group.save()
                    if(receive["retcode"]!=0):
                        print("API Error:"+recv_msg)

@require_websocket
def qqpost_ws_event(req):
    ws_self_id = req.META.get('HTTP_X_SELF_ID')
    ws_client_role = req.META.get('HTTP_X_CLIENT_ROLE')
    ws_access_token = req.META.get('HTTP_AUTHORIZATION').replace("Token","").strip()
    print("%s:%s:%s"%(ws_self_id, ws_access_token, ws_client_role))
    ...
    if(ws_client_role=="Event"):
        while(True):
            ...
            try:
                bot.refresh_from_db()
                recv_msg = recv_msg.decode()
                print("%s Event:%s"%(ws_self_id,recv_msg))
                ...
                if(GLOBAL_EVENT_HANDEL):
                    ...
                    if (receive["post_type"] == "message"):
                        ...
                        if (receive["message"].find('/help')==0):
                            ...
                            msg = msg.strip()
                            bot.send_message(receive["message_type"], group_id or user_id, msg)
                        ...
                        if (receive["message_type"]=="group"):
                                ...
                                if not group.registered:
                                    ...
                                    if(not user_info):
                                        ...
                                        if(receive["message"].find('/set_welcome_msg')==0):
                                            ...
                                            msg = "本群剩余%s条%s聊天限额"%(group.left_reply_cnt,bot.name)
                                            bot.send_message("group", group_id, msg)
                                                                ...
                                                                if(len(vtlist["voted_by"]) >= group.ban_cnt):
                                                                    ...
                                                                    if not target_user_info:
                                                                        ...
                                                                        if(target_user_info["role"]=="owner"):
                                                                            msg = "虽然你是狗群主%s无法禁言，但是也被群友投票禁言，请闭嘴%s分钟[CQ:face,id=14]"%(bot.name, mem.ban_time)
                                                                        if(target_user_info["role"]=="admin"):
                                                                            msg = "虽然你是狗管理%s无法禁言，但是也被群友投票禁言，请闭嘴%s分钟[CQ:face,id=14]"%(bot.name, mem.ban_time)
                                                                        if(str(mem.user_id) == str(receive["self_id"])):
                                                                            ...
                                                                            group_ban(bot,group_id,user_id,int(mem.ban_time)*60)
                                                ...
                                                if(receive["message"]=="/revenge_confirm"):
                                                    for item in qq_list:
                                                        if(str(item)==str(user_id)):
                                                            continue
                                                        group_ban(bot,group_id,item,int(rev.ban_time)*60)
                                                        time.sleep(1)
                                                    time.sleep(1)
                            ...
                            if(len(chats)>0):
                                ...
                                if(group.repeat_ban>0 and chat.times>=group.repeat_ban):
                                    ...
                                    delete_msg(bot,receive["message_id"])
                                    bot.send_message("group", group_id, msg)
                                if(group.repeat_length>=1 and group.repeat_prob>0 and chat.times>=group.repeat_length and (not chat.repeated)):
                                    if(random.randint(1,100)<=group.repeat_prob):
                                        bot.send_message("group", group_id, chat.message)
                                        chat.repeated = True
                                        chat.save()
                            else:
                            ...
                            if("[CQ:at,qq=%s]"%(receive["self_id"]) in receive["message"]):
                                if(group.left_reply_cnt <= 0):
                                    ...
                                    tuling_data["userInfo"] = {"apiKey": TULING_API_KEY if bot.tuling_token=="" else bot.tuling_token, "userId": receive["user_id"], "groupId": group.group_id}
                    ...
                    if (receive["post_type"] == "request"):
                        if (receive["request_type"] == "friend"):   #Add Friend
                            qq = receive["user_id"]
                            flag = receive["flag"]
                            reply_data = {"flag":flag, "approve": bot.auto_accept_friend}
                            bot.send_ws_api("set_friend_add_request",reply_data)
                        if (receive["request_type"] == "group" and receive["sub_type"] == "invite"):    #Add Group
                            ...
                            bot.send_ws_api("set_group_add_request",reply_data)
