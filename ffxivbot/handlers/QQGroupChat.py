...
class QQGroupChat(QQEventHandler):
    def __init__(self, **kwargs):
        super(QQGroupChat, self).__init__()
        ...
        try:
            ...
            if reply_enable:
                ...
                if(len(match_replys)>0):
                    ...
                    msg = item.value
                    msg_action = self.reply_message_action(receive, msg)
            ...
            if(len(chats)>0):
                ...
                if(group.repeat_ban>0 and chat.times>=group.repeat_ban):
                    ...
                    action = self.delete_message_action(receive["message_id"])
                    action_list.append(action)
                    action = self.group_ban_action(group_id, user_id, 60)
                    action_list.append(action)
                    action = self.reply_message_action(receive, msg)
                    action_list.append(action)
                if(group.repeat_length>=1 and group.repeat_prob>0 and chat.times>=group.repeat_length and (not chat.repeated)):
                    if(random.randint(1,100)<=group.repeat_prob):
                        action = self.reply_message_action(receive, chat.message)
                        action_list.append(action)
                        chat.repeated = True
                        chat.save()
            else:
                ...
                for wbt in wbts:
                    if group not in wbt.pushed_group.all():
                        ...
                        if(wbt.crawled_time>=int(time.time())-60*10):
                            action = self.reply_message_action(receive, tmp_msg)
                            action_list.append(action)
                        break
