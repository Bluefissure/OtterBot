...
def QQCommand_fsx(*args, **kwargs):
    try:
        ...
        if '暴击' in s_msg:
            ...
            msg += '暴击 {} 的计算结果(基数:380)：\n暴击率：   {}%\n暴击伤害：   {}%\n预期收益：   {}\n下个变节点:暴击 {}'.format(critical+380,per,bonus,edmg,cri_tmp)
            ...
            while per_tmp == per:
                dir_tmp += 1
                per_tmp = (math.floor(550 * (dir_tmp-340)/nlv))/10
            msg += '直击 {} 的计算结果(基数:380)：\n直击率：    {}%\n直击伤害固定为125%。\n下个变节点:直击 {}'.format(direct+380,per,dir_tmp)
            ...
            msg += '信念 {} 的计算结果(基数:340)：\n伤害增益：{}倍\n下个变节点:信念 {}'.format(determination+340,mult,det_tmp)
            ...
            msg += '坚韧 {} 的计算结果(基数:380)：\n伤害增益：{}倍（仅防护职业）\n受击伤害：{}%\n下个变节点:坚韧 {}'.format(tenacity+380,mult,mit,ten_tmp)
