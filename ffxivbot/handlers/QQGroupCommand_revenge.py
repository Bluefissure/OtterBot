...
class QQGroupCommand_revenge(QQEventHandler):
    def __init__(self, **kwargs):
        super(QQGroupCommand_revenge, self).__init__()
                ...
                if(len(revs)>0):
                    ...
                    for item in qq_list:
                        if(str(item)==str(user_id)):
                            continue
                        ban_action = self.group_ban_action(group_id,item,int(rev.ban_time)*60)
                        action_list.append(ban_action)
                    ban_action = self.group_ban_action(group_id,user_id,int(rev.ban_time)*(len(qq_list))*60)
                    action_list.append(ban_action)
