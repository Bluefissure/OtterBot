...
def QQCommand_comment(*args, **kwargs):
    try:
        ...
        if comment_content.strip().find("reply")==0 and int(user_id)==int(ADMIN_ID):
            ...
            if comm.left_group:
                message = "留言#{}\"{}\"的回复如下：\n======\n{}\n======\n[CQ:at,qq={}]".format(comm.id, comm.content, reply_content, comm.left_by)
                jdata = {
            ...
            async_to_sync(channel_layer.send)(reply_bot.api_channel_name, {"type": "send.event","text": json.dumps(jdata),})
            comm.reply = reply_content
            comm.save(update_fields=["reply"])
        elif comment_content!="":
            comment = Comment(left_by=str(user_id),content=comment_content,left_group=group_id, bot_id=self_id)
            comment.left_time = int(time.time())
            comment.save()
            msg = "留言#{}:\"{}\"添加成功".format(comment.id, comment.content)
            if ADMIN_ID:
                message = "Comment#{} from {} {}:\n{}".format(comment.id, user_id, group_id, comment.content)
                ...
                channel_layer = get_channel_layer()
                async_to_sync(channel_layer.send)(admin_bot.api_channel_name, {"type": "send.event","text": json.dumps(jdata),})
        else:
            msg = "请输入留言内容"
