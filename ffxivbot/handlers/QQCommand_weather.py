    ...
    try:
        ...
        if receive["message"].find("/weather") == 0:
            ...
            if len(territory_name) == 0 or territory_name == "help":
                msg = (
                    "/weather $map ($cnt): 查看对应地图接下来的(几个)天气\n"
                ...
                try:
                    ...
                    for temp in Territory.objects.all():
                        for temp_name in json.loads(temp.nickname):
                            if (
                                temp_name in territory_name
                                or territory_name in temp_name
                            ):
                                territory = temp
                                break
                        if territory:
                ...
                if territory:
                    if len(msg_args) == 1 or str.isdigit(msg_args[1]):
                        ...
                        try:
                            ...
                            cnt = (
                                min(15, int(msg_args[2]))
                                if len(msg_args) >= 3 and str.isdigit(msg_args[2])
                                else 3
                            )
                            times = getSpecificWeatherTimes(
                                territory, weathers, cnt, TIMEFORMAT_MDHMS
                            )
