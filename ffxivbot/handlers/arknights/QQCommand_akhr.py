...
def get_comb_text(hr, all_comb=False):
    ...
    for comb in hr:
        ...
        for i in range(6,0,-1):
            ...
            if lv_i:
                text = " ".join(list(map(lambda x:x["name_cn"], lv_i)))
                msg += "    {}🌟:{}\n".format(i, text)
    return msg.strip()
...
def QQCommand_akhr(*args, **kwargs):
    ...
    try:
        ...
        if len(para_segs) == 0 or para_segs[0] == "help":
            ...
            msg = ""
            valid_tags = ['近卫干员','狙击干员','重装干员','医疗干员','辅助干员','术师干员','特种干员','先锋干员','近战位','远程位','高级资深干员','资深干员','控场','爆发', '治疗','支援','新手','费用回复','输出', '生存', '群攻','防护','减速','削弱', '快速复活', '位移','召唤','支援机械','男性干员','女性干员']
            if img_url:
                if ocr_type=="baidu":
